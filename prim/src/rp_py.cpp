...
extern "C" void rp(uchar *img, uint *imgShape, double *segmentMask,  uint nProposals, double *alpha, uint alphaSize, uint *out) {    

    // Load image
    const Image I(img, std::vector<uint> (imgShape, imgShape + 3), RGB);

    // Execute algorithm in different colorspaces
